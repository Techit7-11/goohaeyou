<script lang="ts">
    ...
    async function submitChanges() {
        ...
        if (response.data?.msg == "CUSTOM_EXCEPTION") {
            rq.msgAndRedirect({ msg: response.data?.data?.message }, undefined, `/applications/detail/${applicationId}`);
        }
        if (response.data?.statusCode === 204) {
            rq.msgAndRedirect({ msg: "수정 완료" }, undefined, `/applications/detail/${applicationId}`);
        } else {
            rq.msgAndRedirect(undefined, { msg: '지원서를 수정하는 중 오류가 발생했습니다.' }, `/applications/detail/${applicationId}`);
        }
    }
</script>
