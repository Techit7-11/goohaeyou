        ...
        <div class="flex flex-col items-center">
            <ul class="w-full max-w-4xl">
                ...
                <div class="flex items-center m-4"> 
                    {#if canApprove}
						<input type="checkbox" id={`application-${application.id}`} class="checkbox checkbox-primary mr-4" on:change={(event) => {event.stopPropagation(); toggleApplicationId(application.id, event.target.checked);}} />
                    {/if}
